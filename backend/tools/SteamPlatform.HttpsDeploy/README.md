...
- `enable` 在改写 Nginx 前分别保存 `sites-available` 与实际加载的 `sites-enabled` 配置；配置校验或公网验证失败时自动恢复两者。
- 两份备份固定保存在 `/var/lib/steam-platform-https/backups/`，不得放入 Nginx 的通配 include 目录。
