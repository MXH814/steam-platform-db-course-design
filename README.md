...
- 游戏库支持搜索、最近游玩筛选、排序、紧凑视图、启动选项、游戏设置和本地笔记；库存支持按游戏、稀有度和状态筛选；市场支持游戏物品/平台物品视图切换；社区评测支持收藏、价值投票、欢乐和奖励状态反馈；创意工坊支持作品搜索、排序、详情和本地订阅管理。
...
| 顺序 | 增强项 | 当前状态 | 完成标准 |
|---|---|---|---|
| 1 | 正式域名和 HTTPS | 暂无自有域名，待选择 `steam-` 相关域名 | 域名解析到腾讯云；80 自动跳转 443；证书自动续期；公网前端、API 和 SignalR 均通过 HTTPS 工作 |
| 2 | 一键恢复演示数据 | 已完成并通过云端重置、恢复、再次重置验收 | .NET 工具先备份、再重置、再校验；操作有确认口令和审计日志；失败可恢复；执行前后健康检查通过 |
| 3 | 持久化增强交互与实时通知 | 待开始 | 好友聊天、评测互动、工坊订阅写入 Oracle；C# 五层接口完整；SignalR 推送消息和状态变化；刷新或换浏览器后状态不丢失 |
| 4 | 商店媒体体验 | 已确定服务器本地托管压缩媒体，待实施 | CS2、DST 详情页具备视频预告片、截图画廊、缩略图切换、全屏查看、键盘操作和加载失败兜底 |
| 5 | 社交与社区扩展 | 待开始 | 好友关系与请求、交易报价、个人资料装扮、徽章、社区动态、讨论主题与回复形成可演示闭环 |
| 6 | 固定答辩演示脚本 | 待开始 | 覆盖 `DST 购买 -> 钱包流水 -> 游戏库 -> 评价与成就 -> 退款` 和 `CS2 入库 -> 库存 -> 挂单 -> 撮合 -> 成交账本`，包含讲解词、账号、预期结果和故障备用步骤 |
| 7 | Playwright 回归与备用录屏 | 待开始 | 桌面与移动视口自动化覆盖登录和两条主演示链路；测试报告可复现；提供 1080p 答辩备用录屏 |
...
1. 当前暂无自有域名；最终域名应与项目名 `steam-` 相关。购买域名、备案或采用临时技术域名前，由总负责人再次选择。
...
- `database/demo/manifest.json`：27 张核心业务表依赖顺序、固定基线和最低行数校验。
...
3. 按该编号运行 `restore`，27 张业务表逐表恢复并通过精确行数校验，运行状态为 `RESTORED`。
...
7. 本地验收通过：恢复工具 4 项测试、后端 166 项测试、数据库 33 项测试、Vue 生产构建。

云端恢复工具版本记录在 `/opt/steam-platform/DEMO_DATA_TOOL_COMMIT`，当前值为 `e2551f9`。连接字符串仍只存在于服务器私有环境变量，不进入仓库、命令参数或日志。
