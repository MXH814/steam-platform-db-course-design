...
1. 启动公告根据公告语义选择配图；第三条“社区创意工坊浏览功能上线”使用独立的饰品创作工坊横幅，不再复用饥荒联机版画面。
2. 未登录状态不再渲染空白客户端底栏，CS2“免费入库”按钮保持完整的单层按钮外观；玩家登录后的下载、添加游戏、好友与聊天底栏功能保持不变。
3. 玩家注册增加确认密码、两组密码可见性按钮和前端一致性校验；确认密码只用于浏览器端校验，不传入后端或数据库。
4. `Survival Lab` 使用独立的野外实验室生存场景，`Tactical Arena` 使用单独的战术竞技画面；两个测试项目均具有自己的展示文案，不再套用饥荒联机版默认视觉。
5. 市场成交记录按当前玩家标记“买入”或“卖出”：买方显示成交支出负数，卖方显示扣除平台手续费后的实际入账正数，并可通过悬停查看成交价、手续费和净入账。
6. 商店搜索改为提交式交互，只有点击搜索按钮或按 Enter 才应用关键词；顶部“库”入口直接进入游戏库，不再展开下拉栏。
7. Vue 类型检查和生产构建通过；本地 Playwright 常规回归共 26 条，其中 24 条通过、2 条需要受控演示数据写入的完整链路按默认规则跳过；部署后公网桌面端与移动端公开商店回归 `8/8` 通过。
8. 前端版本 `e3ac4ea` 已部署到腾讯云，`DEPLOYED_COMMIT=e3ac4ea`；公网 HTTPS 首页、应用健康检查和 Oracle 健康检查均返回 200，`steam-platform-api` 与 Nginx 均为 active。
9. 商店首页保持固定样板游戏优先：精选位不受后来上架的游戏影响，热门游戏卡片保持 `CS2`、`DST` 在前，其他新上架游戏按接口原顺序追加其后；搜索、价格筛选或显式排序时仍尊重用户选择。
10. 游戏库“新情报速递”副标题统一为“来自游戏和社区”，空游戏库不再出现“来自你的”这一不成立的归属表述。
11. 新增首页排序与游戏库文案回归断言；本地及腾讯云公网桌面、移动端相关回归均为 `14/14` 通过。
