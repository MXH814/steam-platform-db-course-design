...
| 6 | 固定答辩演示脚本 | 已完成并通过腾讯云真实业务链验收 | 覆盖 `DST 购买 -> 钱包流水 -> 游戏库 -> 评价与成就 -> 退款` 和 `CS2 入库 -> 库存 -> 挂单 -> 撮合 -> 成交账本`，包含讲解词、账号、预期结果和故障备用步骤 |
| 7 | Playwright 回归与备用录屏 | 已完成并通过腾讯云前后恢复保护验收 | 桌面与移动视口自动化覆盖登录和两条主演示链路；测试报告可复现；提供 1080p 答辩备用录屏 |
...
9. 当前本地验收通过：恢复工具 4 项测试、后端 188 项测试、数据库 36 项测试、Vue 生产构建。
...
- `docs/defense-demo-runbook.md`：12 至 15 分钟主演示、逐步讲解词、固定账号、精确金额、预期数据库结果、加分演示和故障备用步骤。
