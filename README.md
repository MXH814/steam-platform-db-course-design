...
- `database/demo/manifest.json`：33 张业务表依赖顺序、固定基线和最低行数校验，其中包含 27 张核心表和 6 张社交增强表。
...
7. 社交增强接入后，恢复清单扩展到 33 张业务表、113 条基线插入；云端最终基线运行编号为 `202608251417510C1E`，状态为 `RESET_COMPLETED`。
8. 当前本地验收通过：恢复工具 4 项测试、后端 176 项测试、数据库 35 项测试、Vue 生产构建。

云端恢复工具版本记录在 `/opt/steam-platform/DEMO_DATA_TOOL_COMMIT`，当前值为 `a752ba4`。连接字符串仍只存在于服务器私有环境变量，不进入仓库、命令参数或日志。
...
6. `/api/health` 与 `/health/database` 均返回 `OK`；本地后端 176 项、数据库 35 项、恢复工具 4 项测试及 Vue 生产构建通过。
